...
def _noise_reduction_from_cardinal(
...
class TestNoiseReductionAPI:
    ...
    def flat_caches(self, ms_raw_data):
        ...
        caches = []
        for _, intensity_flat, lengths, _ in ms_raw_data.flat_generator(batch_size=4096, include_mz=False, max_threads=16):
    ...
    def test_nr_cardinal_speed(self, benchmark, method, ms_raw_data):
        """Test Cardinal noise reduction speed using Cardinal::smooth."""
        logger.info(f"Benchmarking Cardinal noise reduction method={method}")

        benchmark.pedantic(
            _noise_reduction_from_cardinal,
