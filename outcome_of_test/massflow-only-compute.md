# MassFlow Only-Compute Benchmarks (Python/Numpy vs Parallel/Pipeline)

> 数据来源映射：
> - **Python/Numpy 实现**（batch）= `tests/python_outcome.py` 中每个元组的第一个数值
> - **Parallel/Pipeline 实现**（flat / numba）= `tests/python_outcome.py` 中每个元组的第二个数值
> - **min / mid / max 数据来源**：[`tests/massflow_time&memory_onlyconpute_benchmarks.md`](../tests/massflow_time&memory_onlyconpute_benchmarks.md)（Peak Align 的 time 数据除外，见下）
> - **Peak Align 的 time 数据**（min/mid/max）：[`tests/peak_align_new_test.md`](../tests/peak_align_new_test.md) 的 `## test_align_speed` 与 `## test_align_flat_speed` 表
> - **ultra 数据来源**：[`tests/only_compute_new.md`](../tests/only_compute_new.md)（无内存数据）
> - **单位注意**：源 md 中 Noise Reduction / Normalization / Peak Pick 的 batch 时间为 ms；only_compute_new.md 中所有 ultra 时间为 s
> - Normalization 表中 Reference 行已删除（`python_outcome.py` 只有 TIC/RMS）
...
> 对应 `tests/python_outcome.py` 中每个元组的第一个数值（batch / 原始 NumPy 实现）。

#### Baseline Correction

Time command (min/mid/max):
...
ultra (from `only_compute_new.md`，单位 s):
...
![alt text](../tests/image_of_test/image-10.png)
...
Time command (min/mid/max, in ms):
...
![alt text](../tests/image_of_test/image-12.png)
...
![alt text](../tests/image_of_test/image-13.png)
...
ultra (from `only_compute_new.md`，单位 s)：注意 `python_outcome.py` 使用 `min` 列（46.5643），不是 `mean`：
...
![alt text](../tests/image_of_test/image-14.png)
...
> Time 数据来源：[`tests/peak_align_new_test.md`](../tests/peak_align_new_test.md) 的 `## test_align_speed` 表（取 `min` 方法行，单位 ms）。
> Memory 数据来源：[`tests/massflow_time&memory_onlyconpute_benchmarks.md`](../tests/massflow_time&memory_onlyconpute_benchmarks.md) 的 Peak Align 内存表（batch 行）。
...
![alt text](../tests/image_of_test/image-2.png)
...
### 2. Parallel/Pipeline 实现部分

> 对应 `tests/python_outcome.py` 中每个元组的第二个数值（flat / numba pipeline 实现）。
> 注意：Baseline 与 Peak Pick 的 max 数据由于 example 文件较大时首次 flat 测试出现 cache 异常，`python_outcome.py` 使用了"pipeline 流程测试"的修正表；min/mid 仍取首测表。
...
首测表（min/mid 取用，max 因 cache 不准，单位 ms）：
...
<!-- 注：由于 example 数据较大，flat 测试时需要将数据先 cache，导致结果不准，下面为使用 pipeline 流程测试的结果，稍慢于实际速度 -->
...
![alt text](../tests/image_of_test/image-11.png)
...
ultra (from `only_compute_new.md`，原表为 ms，已换算为 s)：
...
`python_outcome.py` 中 ultra 元组第二个值已统一为 s：MA 0.1923563、Gaussian 0.4965438、Savgol 0.6860692。
...
Time command (min/mid/max, in ms)：表格已删除 Reference 行（`python_outcome.py` 只有 TIC/RMS）：
...
> Time 数据来源：[`tests/peak_align_new_test.md`](../tests/peak_align_new_test.md) 的 `## test_align_flat_speed` 表（取 `min` 方法行，单位 ms）。
> Memory 数据来源：[`tests/massflow_time&memory_onlyconpute_benchmarks.md`](../tests/massflow_time&memory_onlyconpute_benchmarks.md) 的 Peak Align 内存表（flat 行）。
...
![alt text](../tests/image_of_test/image-3.png)
